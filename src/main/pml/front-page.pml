<?xml version="1.0" encoding="utf-8"?>
<document>
    <page>
        <part text="Front Matter"/>

        <label pos="100,650" width="395" align="center" font="hvb" font-size="25">d100 Open-Ended Roleplaying</label>
        <label pos="100,600" width="395" align="center" font="hvb" font-size="25">Bare Metal Edition</label>
        <label pos="100,550" width="395" align="center" font="hvb" font-size="25">(aka. OER/BME)</label>

        <svg ref="toolbox-icon" pos="147,147" width="300" height="300" />
    </page>
    <page>
        <part text="Credits"/>

        <svg ref="tied-scroll-icon" pos="300,700" width="50" height="50" />
        <svg ref="tinker-icon" pos="400,700" width="50" height="50" />
        <svg ref="quill-ink-icon" pos="500,700" width="50" height="50" />

        <markdown pos="50,600" width="495"
                  paragraph-font-size="15"
                  paragraph-font-lead="20"
                  src="../markdown/1-credits.md" />
    </page>
    <page>
        <markdown pos="50,800" width="495"
                  src="../markdown/0-glossary.md" />
    </page>
</document>